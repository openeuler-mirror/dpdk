...
static void hinic3_link_event_stats(void *dev, u8 link)
{
	struct hinic3_hwdev *hwdev = dev;

	if (link)
		rte_atomic32_inc(
			&hwdev->hw_stats.link_event_stats.link_up_stats);
	else
		rte_atomic32_inc(
			&hwdev->hw_stats.link_event_stats.link_down_stats);
}

static void hinic3_set_vport_state(struct hinic3_hwdev *hwdev, struct hinic3_cmd_link_state *link_state)
{
	struct hinic3_nic_dev *nic_dev = NULL;
	int err = 0;

	nic_dev = HINIC3_RTE_ETH_DEV_TO_PRIVATE_NIC_DEV((struct rte_eth_dev *)(hwdev->eth_dev));
				      ...
				      __rte_unused u16 *out_size)
{
	...
	if (!err)
#ifndef DPDK_20_11
		_rte_eth_dev_callback_process(dev->eth_dev,
...
static const char *get_port_type_name(u32 type)
{
	int i;
...
static void get_port_type(struct mag_cmd_event_port_info *port_info, const char **port_type)
{
...
static int get_port_temperature_power(struct mag_cmd_event_port_info *info, char *str)
{
	...
	if (info->sfp_type)
		snprintf(str,  CAP_INFO_MAX_LEN - 1, "%s, rx power: %uuw %uuW %uuW %uuW",
			 arr, info->power[0x0], info->power[0x1],
			 info->power[0x2], info->power[0x3]);
	else
		snprintf(str, CAP_INFO_MAX_LEN - 1, "%s, rx power: %uuW, tx power: %uuW",
			 arr, info->power[0x0], info->power[0x1]);

	return 0;
}

static void print_cable_info(struct mag_cmd_event_port_info *port_info)
{
	...
	memcpy(tmp_vendor, port_info->vendor_name, sizeof(port_info->vendor_name));
	(void)snprintf(tmp_str, CAP_INFO_MAX_LEN - 1,
		       "Vendor: %s, %s, length: %um, max_speed: %uGbps",
		       tmp_vendor, port_type, port_info->cable_length, port_info->max_speed);

	if (port_info->port_type == LINK_PORT_OPTICAL_MM || port_info->port_type == LINK_PORT_AOC) {
		err = get_port_temperature_power(port_info, tmp_str);
		if (err)
			return;
	}

	PMD_DRV_LOG(INFO, "Cable information: %s\n", tmp_str);

	return;
}

static void print_link_info(struct mag_cmd_event_port_info *port_info)
{
	const char *fec = "None";
	...
	if (port_info->speed >= PORT_SPEED_UNKNOWN) {
		PMD_DRV_LOG(INFO, "speed %u is invalid\n", port_info->speed);
		return;
	}
...
static void print_port_info(void *hwdev, struct mag_cmd_event_port_info *port_info, u8 type)
{
	...
	PMD_DRV_LOG(INFO, "alos: %u, rx_los: %u, PCS 64 66b reg: 0x%x, PCS link: 0x%x, MAC link: 0x%x PCS_err_cnt: 0x%x\n",
		 port_info->alos, port_info->rx_los, port_info->pcs_64_66b_reg,
		 port_info->pcs_link, port_info->pcs_mac_link, port_info->pcs_err_cnt);
	PMD_DRV_LOG(INFO, "his_link_machine_state = 0x%08x, cur_link_machine_state = 0x%08x\n",
