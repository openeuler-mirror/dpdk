...
# [2/4] 替换文件内容中的 hinic3 符号
echo "[2/4] Replace symbols in source files..."
find . -type f \
    ! -path './.git/*' \
    -exec file {} \; | grep -E 'text|ASCII' | cut -d: -f1 | while read -r f; do
    sed -i "s/hinic3/${NEW_NAME}/g" "$f" 2>/dev/null || true
    sed -i "s/HINIC3/${NEW_NAME_UPPER}/g" "$f" 2>/dev/null || true
    sed -i "s/Hinic3/${NEW_NAME_CAPITAL}/g" "$f" 2>/dev/null || true
done
echo "[2/4] Symbol replace done"

# [3/4] 重命名含 hinic3 的文件 (从最深层开始)
echo "[3/4] Rename files..."
find . -type f -name '*hinic3*' ! -path './.git/*' -depth | while read -r f; do
    dir=$(dirname "$f")
    base=$(basename "$f")
    newbase=$(echo "$base" | sed "s/hinic3/${NEW_NAME}/g; s/HINIC3/${NEW_NAME_UPPER}/g; s/Hinic3/${NEW_NAME_CAPITAL}/g")
...
# 重命名含 hinic3 的子目录 (从最深层开始)
find . -type d -name '*hinic3*' ! -path './.git/*' -depth | while read -r d; do
    parent=$(dirname "$d")
    base=$(basename "$d")
    newbase=$(echo "$base" | sed "s/hinic3/${NEW_NAME}/g; s/HINIC3/${NEW_NAME_UPPER}/g; s/Hinic3/${NEW_NAME_CAPITAL}/g")
    if [ "$base" != "$newbase" ]; then
