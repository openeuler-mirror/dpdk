...
int
hinic3_dbg_get_rx_cqe_info(void *hwdev, uint16_t q_id, uint16_t idx, void *buf_out, uint16_t *out_size)
{
	struct hinic3_hwdev *dev = (struct hinic3_hwdev *)hwdev;
	struct hinic3_nic_dev *nic_dev = (struct hinic3_nic_dev *)dev->dev_handle;
...
int
hinic3_dbg_get_sq_info(void *dev, u16 q_id,
		       struct hinic3_dbg_sq_info *sq_info, u16 *msg_size)
{
	...
	if (q_id >= nic_dev->num_sqs) {
		PMD_DRV_LOG(ERR, "Inputting tx queue id is larger than actual tx queue number, qid: %d, num_sqs: %d\n",
	...
	sq_info->ci_addr = (volatile u16 *)HINIC3_CI_VADDR(txq->ci_vaddr_base, q_id);
...
int
hinic3_dbg_get_sq_wqe_info(void *dev, u16 q_id, u16 idx, u16 wqebb_cnt, u8 *wqe, u16 *wqe_size)
{
	struct hinic3_hwdev *hwdev = (struct hinic3_hwdev *)dev;
	struct hinic3_nic_dev *nic_dev = (struct hinic3_nic_dev *)hwdev->dev_handle;
	struct hinic3_txq *txq = NULL;
...
int
hinic3_dbg_get_rq_wqe_info(void *dev, u16 q_id, u16 idx, u16 wqebb_cnt, u8 *wqe, u16 *wqe_size)
{
	struct hinic3_hwdev *hwdev = (struct hinic3_hwdev *)dev;
	struct hinic3_nic_dev *nic_dev = (struct hinic3_nic_dev *)hwdev->dev_handle;
	struct hinic3_rxq *rxq = NULL;
	...
	rxq = nic_dev->rxqs[q_id];
	if ((u32)idx + (u32)wqebb_cnt > rxq->q_depth) {
		return -EFAULT;
	}
