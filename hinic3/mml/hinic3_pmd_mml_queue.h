...
#ifndef _HINIC3_PMD_MML_QUEUE
#define _HINIC3_PMD_MML_QUEUE
...
struct nic_tx_task_section {
	...
	union {
		struct {
			unsigned int vport_id : 12; /* when TX direct, output bond id; when RX direct, output function id */
			unsigned int vport_type : 4;
			unsigned int traffic_type : 6;
			unsigned int slave_port_id : 2; /* only used in TX direct, ctrl pkt(LACP\LLDP) output port id */
...
struct nic_tx_sge {
	...
	union {
		struct {
			unsigned int key : 30;	    /* key or unsed */
			unsigned int extention : 1; /* 0:normal, 1:pointer to next SGE */
...
typedef struct tag_l2nic_rx_cqe {
	...
	union {
		struct {			       /* for nic */
			unsigned int first_len : 13;   /* Datalen of the first or middle pkt size. */
			unsigned int last_len : 13;    /* Data len of the last pkt size. */
			unsigned int pkt_num : 5;      /* the number of packet. */
			unsigned int super_cqe_en : 1; /* only this bit = 1, other fileds in this DW is valid. */
		} bs;
...
typedef struct _nic_rq_wqe {
	struct nic_rq_bd_sec buf_desc; /* rx buffer sge. Notes, buf_desc.len limit in bit0~13 */
	unsigned int rsvd0;	       /* rsvd for 16B align */
	struct nic_rq_bd_sec cqe_sect; /* cqe buffer sge. Notes, cqe_sect.len is in unit of 16B and limit in bit0~4 */
	unsigned int rsvd1;	       /* rsvd for unused */
} nic_rq_wqe;
