...
static void cmd_deinit(cmd_adapter_t *adapter)
{
    int i;
...
static int cmd_init(cmd_adapter_t *adapter)
{
    int err;
...
static int cmd_separate(cmd_adapter_t *adapter, const char *buf_in, uint32_t in_size, char **argv)
{
    char *cmd_buf = NULL;
    char *tmp = NULL;
    char *saveptr = NULL;
...
int hinic3_pmd_mml_lib(const char *buf_in, uint32_t in_size, char *buf_out, uint32_t *out_len, uint32_t max_buf_out_len)
{
