...
static int set_dma_attr_entry(__rte_unused struct hinic3_hwdev *hwdev,
...
{
	return set_dma_attr_entry(hwdev, PCIE_MSIX_ATTR_ENTRY, HINIC3_PCIE_ST_DISABLE,
...
static int get_func_misc_info(struct hinic3_hwdev *hwdev)
{
	int err;
...
static int init_mgmt_channel(struct hinic3_hwdev *hwdev)
{
	int err;
...
static void free_mgmt_channel(struct hinic3_hwdev *hwdev)
{
...
int hinic3_init_hwdev(struct hinic3_hwdev *hwdev)
{
	...
	if (hwdev->bifur_mode == HINIC3_BIFUR_MODE_QPOOL) {
		err = hinic3_qpool_cmdqs_init(hwdev);
		if (err) {
			PMD_DRV_LOG(ERR, "Qpool Init cmdq failed");
			goto init_qpool_cmdqs_err;
		}
	} else {
