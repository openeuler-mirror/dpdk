...
#define HINIC3_AF4_CLEAR(val, member)				\
	((val) & (~(HINIC3_AF4_##member##_MASK <<		\
	HINIC3_AF4_##member##_SHIFT)))
...
#define HINIC3_AF5_CLEAR(val, member)				\
	((val) & (~(HINIC3_AF5_##member##_MASK <<		\
	HINIC3_AF5_##member##_SHIFT)))
 ...
 */
static int hwif_ready(struct hinic3_hwdev *hwdev)
{
	u32 addr, attr1;
 ...
 *   The hardware interface of a pci function device
 */
static void get_hwif_attr(struct hinic3_hwif *hwif)
{
...
static enum hinic3_doorbell_ctrl hinic3_get_doorbell_ctrl_status(
...
static enum hinic3_outbound_ctrl hinic3_get_outbound_ctrl_status(
	struct hinic3_hwif *hwif)
{
...
int hinic3_alloc_db_addr(void *hwdev, void **db_base, enum hinic3_db_type queue_type)
{
...
static int wait_until_doorbell_flush_states(struct hinic3_hwif *hwif,
...
static int hinic3_mmap_bar_addr(struct hinic3_hwdev *hwdev)
{
	...
	db_base = mmap(NULL, 4294967296, PROT_READ | PROT_WRITE, MAP_SHARED, fd, page_offset);
	if (!mgmt_reg_base)
...
static int hinic3_get_bar_addr(struct hinic3_hwdev *hwdev)
{
	...
	cfg_bar = HINIC3_IS_VF_DEV(pci_dev) ?
	...
	if (cfg_regs_base == NULL) {
		PMD_DRV_LOG(ERR, "mem_resource addr is null, cfg_regs_base is NULL");
		return -EFAULT;
...
int hinic3_init_hwif(void *dev)
{
	...
	if (err != 0) {
		...
		PMD_DRV_LOG(ERR, "Hw doorbell/outbound is disabled, attr4 0x%x attr5 0x%x\n",
			    attr4, attr5);
		goto hwif_ready_err;
	}
	if (hwdev->bifur_mode == HINIC3_BIFUR_MODE_QPOOL) {
