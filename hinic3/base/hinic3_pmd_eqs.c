...
#define GET_AEQ_ELEM(eq, idx)		((struct hinic3_aeq_elem *) \
 ...
 */
static void set_aeq_ctrls(struct hinic3_eq *eq)
{
...
static int set_eq_pages(struct hinic3_eq *eq)
{
	struct hinic3_hwif *hwif = eq->hwdev->hwif;
	u32 reg, init_val;
	u16 pg_num, i;
	...
	for (pg_num = 0; pg_num < eq->num_pages; pg_num++) {
		eq->eq_mz[pg_num] = hinic3_dma_zone_reserve(eq->hwdev->eth_dev,	"eq_mz", eq->q_id,
			eq->page_size, eq->page_size, SOCKET_ID_ANY);
		if (!eq->eq_mz[pg_num]) {
			err = -ENOMEM;
			goto dma_alloc_err;
		}
		...
		hinic3_hwif_write_reg(hwif, reg, upper_32_bits(eq->dma_addr[pg_num]));

		reg = HINIC3_AEQ_LO_PHYS_ADDR_REG(pg_num);
		hinic3_hwif_write_reg(hwif, reg, lower_32_bits(eq->dma_addr[pg_num]));
	}
...
static int alloc_eq_pages(struct hinic3_eq *eq)
{
	...
	eq->dma_addr = rte_zmalloc("eq_dma", dma_addr_size, HINIC3_MEM_ALLOC_ALIGN_MIN);
	if (!eq->dma_addr)
		return -ENOMEM;

	eq->virt_addr = rte_zmalloc("eq_va", virt_addr_size, HINIC3_MEM_ALLOC_ALIGN_MIN);
	if (!eq->virt_addr) {
		err = -ENOMEM;
		goto virt_addr_alloc_err;
	}

	eq->eq_mz = rte_zmalloc("eq_mz", eq_mz_size, HINIC3_MEM_ALLOC_ALIGN_MIN);
	if (!eq->eq_mz) {
		err = -ENOMEM;
...
static u32 get_page_size(struct hinic3_eq *eq)
{
	u32 total_size;
	u16 count, n = 0;
 ...
 */
static int init_aeq(struct hinic3_eq *eq, struct hinic3_hwdev *hwdev,
		    u16 q_id, u32 q_len)
{
	int err = 0;
 ...
 *   The event queue
 */
static void remove_aeq(struct hinic3_eq *eq)
{
...
static int aeq_elem_handler(struct hinic3_eq *eq, u32 aeqe_desc,
			    struct hinic3_aeq_elem *aeqe_pos, void *param)
{
	...
	if (event == HINIC3_MSG_FROM_MGMT_CPU)
		return hinic3_mgmt_msg_aeqe_handler(eq->hwdev,
				data, size, param);
	else if (event == HINIC3_MBX_FROM_FUNC)
		return hinic3_mbox_func_aeqe_handler(eq->hwdev, data,
						size, param);
	else {
