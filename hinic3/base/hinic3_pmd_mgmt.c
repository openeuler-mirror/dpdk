...
#define MAX_PF_MGMT_BUF_SIZE 4096UL
			      ...
			      void *buf_in, u16 in_size)
{
	...
	return hinic3_send_mbox_to_mgmt_no_ack(hwdev, mod, cmd,
					       buf_in, in_size);
}
...
static void hinic3_mgmt_recv_msg_handler(
				...
				__rte_unused void *param)
{
	...
	case HINIC3_MOD_HILINK:
		hinic3_pf_mag_event_handler(pf_to_mgmt->hwdev, pf_to_mgmt,
					    recv_msg->cmd, recv_msg->msg,
					    recv_msg->msg_len,
					    buf_out, &out_size);
		break;
 ...
 */
static int recv_mgmt_msg_handler(struct hinic3_msg_pf_to_mgmt *pf_to_mgmt,
...
int hinic3_mgmt_msg_aeqe_handler(void *hwdev, u8 *header, u8 size, void *param)
{
	...
	if ((HINIC3_MSG_HEADER_GET(*(u64 *)header, SOURCE) ==
		...
		return hinic3_mbox_func_aeqe_handler(hwdev, header,
						     size, param);
	}
