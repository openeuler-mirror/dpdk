...
int hinic3_func_reset(void *hwdev, u64 reset_flag)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_COMM,
				      HINIC3_MGMT_CMD_FUNC_RESET, &func_reset,
...
int hinic3_convert_rx_buf_size(u32 rx_buf_sz, u32 *match_sz)
{
	...
	if (rx_buf_sz >= HINIC3_RX_BUF_SIZE_4K) {
		best_match_sz = ((rx_buf_sz >> RX_BUF_SIZE_1K_LEN) << RX_BUF_SIZE_1K_LEN);
		goto size_matched;
	}
...
int hinic3_set_root_ctxt(void *hwdev, u32 rq_depth, u32 sq_depth, u16 rx_buf_sz)
{
...
int hinic3_set_cmdq_depth(void *hwdev, u16 cmdq_depth)
{
	...
	if (((struct hinic3_hwdev *)hwdev)->cmdqs->cmdq_mode == HINIC3_ENHANCE_CMDQ)
