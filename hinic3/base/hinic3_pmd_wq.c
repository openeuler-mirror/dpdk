...
static void free_wq_pages(struct hinic3_wq *wq)
{
...
static int alloc_wq_pages(struct hinic3_hwdev *hwdev, struct hinic3_wq *wq,
			  int qid)
{
...
void hinic3_put_wqe(struct hinic3_wq *wq, int num_wqebbs)
{
	...
	rte_atomic32_add(&wq->delta, num_wqebbs);
}
...
void *hinic3_read_wqe(struct hinic3_wq *wq, int num_wqebbs, u16 *cons_idx)
{
	...
	if ((rte_atomic32_read(&wq->delta) + num_wqebbs) > wq->q_depth)
		return NULL;
		      ...
		      u32 wq_buf_size, u32 wqebb_shift, u16 q_depth)
{
	...
	for (i = 0; i < cmdq_blocks; i++) {
		...
		wq[i].cons_idx = 0;
		wq[i].prod_idx = 0;
		rte_atomic32_set(&(wq[i].delta), q_depth);
