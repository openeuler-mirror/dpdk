			   ...
			   void *buf_out, u16 *out_size)
{
	...
	if (!IS_QPOOL_MODE((struct hinic3_hwdev *)hwdev) && cmd_to_pf) {
...
int hinic3_set_mac(void *hwdev, const u8 *mac_addr, u16 vlan_id, u16 func_id)
{
	...
	if (IS_QPOOL_MODE((struct hinic3_hwdev *)hwdev)) {
 	...
 	if (IS_BIFUR_MODE((struct hinic3_hwdev *)hwdev)) {
...
void hinic3_clear_qid_mmap(u16 q_id)
{
	...
	TAILQ_FOREACH(entry, &g_qid_lqid_list, entries) {
		if (entry->q_id == q_id) {
			TAILQ_REMOVE(&g_qid_lqid_list, entry, entries);
			rte_free(entry);
			return;
		}
	}
...
int hinic3_rss_get_indir_tbl(void *hwdev, u32 *indir_table, u32 indir_table_size)
{
	...
	if (IS_QPOOL_MODE((struct hinic3_hwdev *)hwdev)) {
		...
		if (is_sp230_nic(nic_dev))
			...
			cmd = hinic3_prepare_cmd_buf_get_rss_indir_table_stn(nic_dev, cmd_buf);
		err = hinic3_cmdq_detail_resp(hwdev, HINIC3_MOD_L2NIC, cmd, cmd_buf, cmd_buf, 0);
	}
	if (err || cmd_buf->buf == NULL) {
	...
	if (is_sp230_nic(nic_dev)) 
		hinic3_cmd_buf_to_rss_indir_table_htn((struct hinic3_hwdev *)hwdev, cmd_buf, indir_table, indir_table_size);
	else
		hinic3_cmd_buf_to_rss_indir_table_stn((struct hinic3_hwdev *)hwdev, cmd_buf, indir_table, indir_table_size);
...
int hinic3_rss_set_indir_tbl(void *hwdev, const u32 *indir_table, u32 indir_table_size)
{
	...
	if (is_sp230_nic(nic_dev))
		cmd = hinic3_prepare_cmd_buf_set_rss_indir_table_htn(nic_dev, indir_table, cmd_buf, indir_table_size);
	else
		cmd = hinic3_prepare_cmd_buf_set_rss_indir_table_stn(nic_dev, indir_table, cmd_buf, indir_table_size);
...
int hinic3_set_rss_type(void *hwdev, struct hinic3_rss_type rss_type)
{
	...
	if (IS_QPOOL_MODE((struct hinic3_hwdev *)hwdev) && !is_sp230_nic(nic_dev))
...
int hinic3_get_rss_type(void *hwdev, struct hinic3_rss_type *rss_type)
{
	...
	if (IS_QPOOL_MODE((struct hinic3_hwdev *)hwdev) && !is_sp230_nic(((struct hinic3_hwdev *)hwdev)->dev_handle))
				 ...
				 u16 in_size, void *buf_out, u16 *out_size)
{
	...
	if (!IS_QPOOL_MODE((struct hinic3_hwdev *)hwdev) && hinic3_func_type(hwdev) == TYPE_VF) {
		for (i = 0; i < cmd_cnt; i++) {
...
int hinic3_qinfo_type_detect(const char *dev_file, struct rte_pci_device *pci_dev)
{
	if (access(dev_file, F_OK) == 0)
		return HINIC3_QINFO_TYPE_QPOOL;

	if (access(BIFUR_GDEV_PATH, F_OK) == 0 && (!is_sp230_pci_dev(pci_dev)))
		return HINIC3_QINFO_TYPE_BIFUR;

	return HINIC3_QINFO_TYPE_NORMAL;
}
