...
static inline void hinic3_fdir_ipv6_tcam_key_init_dip(struct hinic3_fdir_filter *rule,
					       struct hinic3_tcam_key *tcam_key)
{
...
int
hinic3_set_fdir_ethertype_filter(void *hwdev, u8 pkt_type, void *filter, u8 en)
{
...
void hinic3_tcam_index_free(void *dev, u16 index, u16 block_id)
{
	...
	if (tmp->dynamic_index_cnt ==0) {
		(void)hinic3_free_tcam_block(nic_dev->hwdev, &block_id);
				  ...
				  bool is_hairpin)
{
	...
	{
		err = hinic3_add_tcam_rule_by_kernel(nic_dev->hwdev, fdir_tcam_rule,
				    ...
				    bool add)
{
	...
	if ((hinic3_get_driver_feature(nic_dev) & NIC_F_HTN_FDIR) && (fdir_filter->action == RTE_FLOW_ACTION_TYPE_RSS)) {
		fdir_tcam_rule.data.dw0.bs.qid = fdir_filter->rss_group_id;
		fdir_tcam_rule.data.dw0.bs.flag = 1;
		fdir_tcam_rule.data.dw1.queue_num = fdir_filter->queue_num;
	} else {
		fdir_tcam_rule.data.dw0.qid = fdir_filter->rq_index;
		fdir_tcam_rule.data.dw1.queue_num = fdir_filter->queue_num;
	}
...
static int hinic3_flow_set_arp_filter(struct rte_eth_dev *dev,
...
static int hinic3_flow_ethertype_rule_nums(struct hinic3_ethertype_filter *ethertype_filter)
{
	switch (ethertype_filter->ether_type) {
	case RTE_ETHER_TYPE_ARP:
