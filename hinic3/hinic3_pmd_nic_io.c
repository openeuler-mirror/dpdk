...
static int hinic3_cmd_modify_tx_queue_ctx(struct hinic3_nic_dev *nic_dev,
				          struct hinic3_qp_ctxt_block_htn *sq_ctxt_block,
					  int qid, int fd)
{
	...
	fill_ioctl_msg(&msg_to_kernel, SEND_TO_NPU, 0, sizeof(struct hinic3_qp_ctxt_block_htn),
		       sizeof(struct hinic3_qp_ctxt_block_htn), sq_ctxt_block, sq_ctxt_block);
...
static int init_sq_ctxts(struct hinic3_nic_dev *nic_dev)
{
	...
	while (q_id < nic_dev->num_sqs) {
		...
		if (nic_dev->hwdev->bifur_mode != HINIC3_BIFUR_MODE_QPOOL)
			...
			err = hinic3_cmd_modify_tx_queue_ctx(nic_dev, cmd_buf->buf, nic_dev->txqs[q_id]->local_qid, nic_dev->fd);
		if (err || out_param != 0) {
...
static int hinic3_cmd_modify_rx_queue_ctx(struct hinic3_nic_dev *nic_dev,
				          struct hinic3_qp_ctxt_block_htn *rq_ctxt_block,
					  int qid, int fd)
{
	struct msg_module msg_to_kernel = { 0 };
	int err;
	...
	msg_to_kernel.lcore_id = nic_dev->global_id;
	fill_ioctl_msg(&msg_to_kernel, SEND_TO_NPU, 0, sizeof(struct hinic3_qp_ctxt_block_htn),
		       sizeof(struct hinic3_qp_ctxt_block_htn), rq_ctxt_block, rq_ctxt_block);
	msg_to_kernel.npu_cmd.direct_resp = 1;
	msg_to_kernel.npu_cmd.mod = HINIC3_MOD_L2NIC;
	msg_to_kernel.npu_cmd.cmd = HINIC3_HTN_CMD_SQ_RQ_CONTEXT_MULTI_ST;
	msg_to_kernel.npu_cmd.ack_type = HINIC3_ACK_TYPE_CMDQ;
...
static int init_rq_ctxts(struct hinic3_nic_dev *nic_dev)
{
	...
	while (q_id < nic_dev->num_rqs) {
		...
		if (nic_dev->hwdev->bifur_mode != HINIC3_BIFUR_MODE_QPOOL)
			...
			err = hinic3_cmd_modify_rx_queue_ctx(nic_dev, cmd_buf->buf, nic_dev->rxqs[q_id]->local_qid, nic_dev->fd);
		if (err || out_param != 0) {
