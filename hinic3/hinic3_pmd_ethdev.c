			...
			struct rte_mempool *mp)
{
	...
	rq_depth = (nb_desc & (nb_desc - 1)) ? ((u16)(1U << (ilog2(nb_desc) + 1))) : nb_desc;
			   ...
			   uint16_t qid, unsigned int socket_id)
{
	...
	/* TSO requires multi-segment SGE path: when any TSO offload is
	 * enabled, force multi_segs=true regardless of MULTI_SEGS flag.
	 */
	txq->multi_segs = (dev->data->dev_conf.txmode.offloads &
			   (RTE_ETH_TX_OFFLOAD_MULTI_SEGS |
			    RTE_ETH_TX_OFFLOAD_TCP_TSO |
			    ...
			    RTE_ETH_TX_OFFLOAD_IP_TNL_TSO)) ? true : false;
...
#endif
{
	...
	/* sp230 and sp560 vf function not support dcb cmd */
	if (nic_dev->dcb->dcb_on) {
		if (is_sp620_nic(nic_dev) ||
...
static int hinic3_func_init_qpool(struct rte_eth_dev *eth_dev)
{
	...
	if (nic_dev->fd < 0) {
		PMD_DRV_LOG(ERR, "Qpool func init get nic fd failed, fd: %d", nic_dev->fd);
		err = nic_dev->fd;
