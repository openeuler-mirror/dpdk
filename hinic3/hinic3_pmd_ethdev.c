...
struct netdev_event {
	...
	u8 dev_addr[HINIC3_DEV_ETHER_ADDR_LEN];
	u16 new_mtu;
};
	...
	while (true) {
		...
		while ((bytes_read = read(intr_handle->fd, &event, sizeof(event))) == sizeof(event)) {
			...
			case NETDEV_UP_EVENT:
				...
				break;
			case NETDEV_DOWN_EVENT:
				get_port_info(nic_dev->hwdev, 0, &link);
				rte_eth_linkstatus_set(dev, &link);
				break;
			case NETDEV_MTUCHANGE_EVENT:
				nic_dev->mtu_size = event.new_mtu;
				dev->data->mtu = event.new_mtu;
				break;
			case NETDEV_MACCHANGE_EVENT:
				memmove(addr_bytes, event.dev_addr, RTE_ETHER_ADDR_LEN);
...
static int hinic3_dev_set_link_up(struct rte_eth_dev *dev)
{
	...
	int err;
	if (IS_QPOOL_MODE(nic_dev)) {
		PMD_DRV_LOG(WARNING, "Qpool not support set link up, fellow kernel");
		return 0;
	}
...
static int hinic3_dev_set_link_down(struct rte_eth_dev *dev)
{
	...
	int err;
	if (IS_QPOOL_MODE(nic_dev)) {
		PMD_DRV_LOG(WARNING, "Qpool not support set link down, fellow kernel");
		return 0;
	}
...
static int hinic3_get_rx_user_queue(struct hinic3_nic_dev *nic_dev, struct hinic3_rxq *rxq)
{
	...
	fill_ioctl_msg(&msg_to_kernel, SEND_TO_BIFUR_DRIVER, GET_USER_QUEUE_ID,
...
static int hinic3_dev_set_mtu(struct rte_eth_dev *dev, uint16_t mtu)
{
	...
	int err = 0;
	if (IS_QPOOL_MODE(nic_dev)) {
		PMD_DRV_LOG(WARNING, "Qpool not support set set mtu, fellow kernel");
		return 0;
	}
