...
parse_skip_ip6_ext(struct rte_ipv6_hdr *ipv6_hdr,
	...
	for (i = 0; i < MAX_EXT_HDRS; i++) {
		switch (proto) {
			...
			case IPPROTO_DSTOPTS:
				xh = (struct ext_hdr *)((uint8_t *)ipv6_hdr +
							info->l3_len);
				info->l3_len += (xh->len + 1) << 3;
				proto = xh->next_hdr;
				break;
			case IPPROTO_AH:
				xh = (struct ext_hdr *)((uint8_t *)ipv6_hdr +
							info->l3_len);
				info->l3_len += (xh->len + 2) << 2;
				proto = xh->next_hdr;
				break;
...
parse_ipv6(struct rte_ipv6_hdr *ipv6_hdr, struct testpmd_offload_info *info)
{
	...
	info->l4_proto = parse_skip_ip6_ext(ipv6_hdr, info);
...
static uint16_t
hinic3_ipv6_phdr_cksum(const struct rte_ipv6_hdr *ipv6_hdr, uint64_t ol_flags, uint16_t l3_len)
{
	uint32_t sum;
	...
	if (ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
		...
		if ( (ol_flags & RTE_MBUF_F_TX_L4_MASK) == RTE_MBUF_F_TX_L4_MASK)
	...
	if (info->l4_proto == IPPROTO_UDP) {
		...
		if (tso_segsz) {
			...
			if (ol_flags & RTE_MBUF_F_TX_IPV4)
				...
				tcp_hdr->cksum = hinic3_ipv6_phdr_cksum(ipv6_hdr, ol_flags, info->l3_len);
